...
const contactFormSchema = z.object({
  ...
  landAreaSqft: z.string().min(1, 'Land area is required'),
    ...
    try {
      ...
      const response = await fetch(cmsUrl('/api/contact-entries'), {
        ...
        body: JSON.stringify({
          ...
          land_area_sqft: parseInt(data.landAreaSqft),
          land_address: data.landAddress,
        }),
      })
        ...
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-lg bg-white p-8 shadow-lg dark:bg-gray-800">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              {/* Land Area (sqft) */}
              <div>
                <Label htmlFor="landAreaSqft">Land Area (sqft) *</Label>
                <Input
                  id="landAreaSqft"
                  type="text"
                  {...register('landAreaSqft')}
                  className="mt-1"
                  placeholder="Enter land area in square feet"
                />
                {errors.landAreaSqft && (
                  <p className="mt-1 text-sm text-red-600">{errors.landAreaSqft.message}</p>
                )}
              </div>
